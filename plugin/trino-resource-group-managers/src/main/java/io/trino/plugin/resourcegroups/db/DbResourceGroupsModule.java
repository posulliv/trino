        ...
        implements Module
{
    ...
    public void configure(Binder binder)
    {
        ...
        binder.bind(FlywayMigration.class).asEagerSingleton();
        binder.bind(DatabaseReady.class).to(FlywayMigration.class).in(Scopes.SINGLETON);
        newExporter(binder).export(DbResourceGroupConfigurationManager.class).withGeneratedName();
    }

    @Provides
    @Singleton
    public static Jdbi create(DbResourceGroupConfig config, @SuppressWarnings("unused") DatabaseReady ignored)
    {
