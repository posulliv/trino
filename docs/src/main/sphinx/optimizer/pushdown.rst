...
* join pushdown must be enabled in a catalog
...
:doc:`EXPLAIN plan </sql/explain>` of the query. The explain plan does not
show a ``Join`` operator, if the join is pushed down to the data source by the
connector.

Prepend ``EXPLAIN`` to a query to access the explain plan::
...
Pushing down a join can increase the row count compared to the size of the
input to the join. This may incur costs due to increased network traffic.
